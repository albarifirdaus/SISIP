      (() => {
        ...
        function safeImage(value) { return /^(data:image\/(png|jpe?g|webp|gif);base64,|https?:\/\/)/i.test(String(value || "")) ? String(value) : ""; }
        ...
        function imageFrameClass(value, fallback = "portrait") { return "image-frame--" + imageAspect(value, fallback); }
        ...
        function hasMemberMatchSignals(preference = memberPreference()) {
          return Boolean(preference.genderTarget || preference.styleTags.length || preference.budgetMin || preference.budgetMax);
        }
        ...
        function renderPersonalized() {
          ...
          const preference = memberPreference();
          if (!preference.onboardingCompleted || !hasMemberMatchSignals(preference)) {
        ...
        function openLook(id, { navigate = true } = {}) {
          ...
          const itemsHtml = entry.items.map((item)=>{
            if (!item?.productId) {
              const itemAction=item.affiliateUrl?`<a class="item-link" href="${esc(safeUrl(item.affiliateUrl))}" target="_blank" rel="sponsored noopener" data-insight-target="curator_item" data-insight-id="${esc(item.id||"")}">${esc(marketplaceLabel(item))} ↗</a>`:`<span class="item-link" aria-disabled="true">Link nonaktif</span>`;
