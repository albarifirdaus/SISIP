      (() => {
        ...
        function renderPersonalized() {
          ...
          if ((!preference.onboardingCompleted || !hasMemberMatchSignals(preference)) && !memberRetention.hasSignals()) {
            els.personalGrid.innerHTML = `<div class="personal-intro"><div><p class="eyebrow">HALO, ${esc(memberDisplayName().toUpperCase())}</p><h3>Style apa yang kamu suka?</h3></div><p>Pilih minimal satu style, gender preferensi, atau budget. Kamu bisa mengubahnya kapan saja.</p><button class="button-outline" type="button" data-open-member>Isi preferensi ↗</button></div><div class="personal-results"><p class="personal-empty">Setelah disimpan, COMOOTD akan mencocokkan tag katalog dan menampilkan Look serta produk yang paling dekat dengan preferensimu.</p></div>`;
            return;
          }
          ...
          els.personalGrid.innerHTML = `<div class="personal-intro"><div><p class="eyebrow">UNTUK ${esc(memberDisplayName().toUpperCase())}</p><h3>Pilihan berdasarkan style dan aktivitasmu.</h3></div><p>Urutan memakai profil, item tersimpan, Curator yang diikuti, dan riwayat yang hanya terlihat olehmu.</p><button class="button-outline" type="button" data-open-member>Atur profil &amp; koleksi ↗</button></div><div class="personal-results"><div class="personal-group"><div class="personal-group-head"><h4>Look yang mungkin kamu suka</h4><span class="eyebrow">${matches.looks.length} pilihan</span></div><div class="personal-looks">${looks || `<p class="personal-empty">Belum ada look yang cukup cocok.</p>`}</div></div><div class="personal-group"><div class="personal-group-head"><h4>Pieces yang mungkin kamu suka</h4><span class="eyebrow">${matches.products.length} pilihan</span></div><div class="personal-products">${products || `<p class="personal-empty">Belum ada produk yang cukup cocok.</p>`}</div></div></div>`;
        }
        ...
        function renderNewSeries() {
          ...
          if (slide.kind === "campaign") {
            ...
            els.newSeriesStage.innerHTML = `<button class="new-series-slide" type="button" data-open-look="${esc(entry.id)}" aria-label="Buka look ${esc(entry.title)}"><span class="new-series-visual">${lookVisual(entry,true)}</span><span class="new-series-overlay"><span class="new-series-count meta">${count}</span><strong>${esc(entry.title)}</strong><span class="new-series-meta">${esc(entry.gender)} · ${esc(styles)}</span><span class="new-series-open">Buka look ↗</span></span></button>`;
          }
        ...
        function renderJournalCtaList(type) {
          ...
          target.innerHTML = entries.length ? entries.map((entry,index) => {
            ...
            const owner = type === "look" ? `<span class="journal-cta-owner">${esc(journalLookOwnerLabel(item))}</span>` : "";
            return `<div class="journal-cta-item${type === "look" ? " is-look" : ""}"><div>${type === "look" && item ? `<span class="journal-cta-thumb" aria-hidden="true">${lookVisual(item)}</span>` : ""}<p><strong title="${esc(item?.title || item?.name || "Kurasi")}">${esc(item?.title || item?.name || "Kurasi")}</strong>${owner}</p></div><input maxlength="80" data-journal-${type}-cta-label="${index}" value="${esc(entry.label || defaultJournalCtaLabel(type))}" aria-label="Label CTA ${type}" /><button class="small-button danger" type="button" ${removeKey}="${index}" aria-label="Hapus CTA">×</button></div>`;
          }).join("") : `<p class="microcopy" style="color:var(--taupe);margin:0">Belum ada ${type === "look" ? "look" : "produk"} terkait.</p>`;
        }
        function journalLookOwnerLabel(entry) {
          const curatorName = entry?.curator?.displayName || entry?.curator?.name || "";
          const curatorHandle = String(entry?.curator?.handle || "").trim();
          return curatorName ? `CURATED BY ${curatorName}${curatorHandle ? ` · @${curatorHandle}` : ""}` : "BY COMOOTD";
        }
        function journalLookOptions(looks, selectedIds) {
          const available = looks.filter((entry) => !selectedIds.has(entry.id));
          const curatorLooks = available.filter((entry) => entry.publisherType === "curator" || entry.curator?.handle);
          const comootdLooks = available.filter((entry) => !curatorLooks.includes(entry));
          const option = (entry) => `<option value="${esc(entry.id)}">${esc(entry.title)} · ${esc(journalLookOwnerLabel(entry))}</option>`;
          return [
            curatorLooks.length ? `<optgroup label="LOOKS DARI CURATOR">${curatorLooks.map(option).join("")}</optgroup>` : "",
            comootdLooks.length ? `<optgroup label="LOOKS DARI COMOOTD">${comootdLooks.map(option).join("")}</optgroup>` : ""
          ].join("");
        }
        function renderJournalCuration() {
          ...
          els.journalLookCtaInput.innerHTML = `<option value="">Pilih Looks terbit dari semua kurator</option>${journalLookOptions(looks, selectedLookIds)}`;
        ...
        function renderJournal() {
          document.getElementById("journalGrid").innerHTML = state.articles.length ? state.articles.slice(0,12).map((article) => {
            ...
            return `<article class="journal-card">${cover ? `<div class="journal-card-cover ${imageFrameClass(article.coverAspect || article.coverImage, "portrait")}" aria-hidden="true"><img ${window.COMOOTDImageOptimizer?.attributes(cover)||''} src="${esc(cover)}" alt="" /></div>` : ""}<span class="article-number eyebrow">${esc(articleCategoryLabel(article.category))} / ${esc(article.number)}</span><h3>${esc(article.title)}</h3>${article.excerpt ? `<p class="journal-card-excerpt">${esc(article.excerpt)}</p>` : ""}<button class="text-link" type="button" data-open-article="${esc(article.id)}">Baca artikel ↗</button></article>`;
        ...
        async function openLook(id, { navigate = true } = {}) {
          ...
          const itemsHtml = entry.items.map((item)=>{
            if (!item?.productId) {
              ...
              return `<li class="look-item"><div class="look-item-art look-item-placeholder" aria-hidden="true"><span>${esc(item.category||"Item")}</span></div><div class="look-item-copy"><p class="look-item-title">${esc(item.name||"Item pilihan")}</p><div class="look-item-info"><span>${esc(item.colorLabel||item.variantName||"Warna pilihan")}</span></div><p class="look-item-price">${Number(item.price)>0?money(item.price):"Cek harga di toko"}</p></div><div class="look-item-actions">${itemAction}</div></li>`;
            }
            const productItem=getProduct(item.productId); if (!productItem) return "";
            const variant=getVariant(productItem,item.variantName);
            return `<li class="look-item"><button class="look-item-art product-card-open" type="button" data-open-product="${esc(productItem.id)}" aria-label="Buka detail ${esc(productItem.name)}">${productArt(productItem,variant,true)}</button><div class="look-item-copy"><button class="look-item-title" type="button" data-open-product="${esc(productItem.id)}">${esc(productItem.name)}</button><div class="look-item-info"><span class="swatch" style="--swatch:${esc(variant?.hex||"#ccc")}"></span><span>${esc(variant?.name||"Warna pilihan")}</span></div><p class="look-item-price">${Number(productItem.price)>0?money(productItem.price):"Cek harga di toko"}</p></div><div class="look-item-actions">${marketplaceDestinationsMarkup(productItem,{contextLook:entry.id,reportType:"product_link",className:"marketplace-destination"})}</div></li>`;
          }).join("");
          const curatorLine = ""; // Attribution is already shown above the title.
        ...
        async function openProduct(id, { navigate = true, variantId = "", replace = false } = {}) {
          ...
          const relatedLooksMarkup=relatedLooks.length ? `<section class="product-related-looks"><h3>See It in a Look</h3><div class="product-related-look-list">${relatedLooks.map((look)=>`<button class="product-related-look" type="button" data-open-product-look="${esc(look.id)}">${lookVisual(look)}<span class="product-related-look-copy"><span>${esc(lookAttribution(look))}</span><strong>${esc(look.title)}</strong></span></button>`).join("")}</div></section>` : "";
          els.productDetail.innerHTML = `<div class="product-detail"><div>${productArt(entry, selectedVariant,false,true)}</div><div><p class="eyebrow" style="color:var(--clay)">COMOOTD / ${esc(gender)}</p><h2>${esc(entry.name)}</h2>${entry.badge ? `<p class="eyebrow" style="color:var(--taupe)">${esc(entry.badge)}</p>` : ""}<p class="product-detail-price">${money(entry.price)}</p><div class="look-card-tags">${(entry.styles || []).map((style) => `<span class="tag">${esc(style)}</span>`).join("")}</div>${variants ? `<div class="product-variant-options" aria-label="Pilih warna">${variants}</div>` : ""}<div class="detail-actions">${marketplaceDestinationsMarkup(entry,{reportType:"product_link"})}${memberRetention.saveButton("product",entry.id)}<button class="button-outline" type="button" data-share-product="${esc(entry.id)}" data-share-variant="${esc(selectedVariant?.id || "")}">Bagikan</button></div><p class="price-note">Harga referensi saat kurasi. Warna, stok, dan harga akhir dapat berubah di marketplace.</p>${relatedLooksMarkup}</div></div>`;
        ...
        function articleCurationMarkup(article) {
          ...
          const lookMarkup = looks.length ? `<div class="article-related-looks">${looks.map((cta) => { const look=cta.look; return `<button class="article-related-look" type="button" data-open-article-look="${esc(look.id)}" aria-label="Buka ${esc(look.title)}, ${esc(journalLookOwnerLabel(look))}"><span class="article-related-look-visual">${lookVisual(look)}</span><span class="article-related-look-copy"><small>${esc(cta.label || defaultJournalCtaLabel("look"))}</small><strong>${esc(look.title)}</strong><em>${esc(journalLookOwnerLabel(look))}</em><span>${(look.styles || []).slice(0,2).map((style)=>`<b>${esc(style)}</b>`).join("")}<b>${look.items?.length || 0} ITEM</b></span></span><span class="article-related-look-arrow" aria-hidden="true">↗</span></button>`; }).join("")}</div>` : "";
          ...
          return `<section class="article-curation"><p class="eyebrow">LOOKS REFERENSI</p><h3>Explore the Looks</h3>${lookMarkup}${productMarkup}</section>`;
        }
