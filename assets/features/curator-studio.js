...
(() => {
  ...
  function controlledTagPickerMarkup({ name, options, selected, maximum = 3, label, note = "" }) {
    // Preserve existing choices until the curator explicitly replaces them.
    const preserved = tagList(selected, 24);
    const available = [...new Set([...options, ...preserved])];
    const chosen = new Set(controlledTagList(selected, available, available.length));
    return `<fieldset class="curator-choice-picker" data-curator-choice-picker data-max-selections="${maximum}">
      ...
      <div class="curator-choice-list" hidden>${available.map((option) => `<label class="curator-choice${chosen.has(option) ? " is-selected" : ""}"><input type="checkbox" name="${esc(name)}" value="${esc(option)}" data-style-current="${options.includes(option)}"${chosen.has(option) ? " checked" : ""} /><span>${esc(option)}</span></label>`).join("")}</div>
  ...
  function onboardMarkup() {
    ...
    return `<div class="curator-onboard-shell"><button class="icon-button modal-close" type="button" data-close-curator-onboard aria-label="Tutup">×</button><p class="eyebrow" style="color:var(--clay)">COMOOTD / CURATOR APPLICATION</p><h2>Show Your<br />Point of View.</h2><p class="curator-onboard-copy">Akun Curator hanya diaktifkan setelah peninjauan. Kami melihat sudut pandang, konsistensi konten, dan apakah referensimu membantu orang menemukan outfit dengan lebih mudah.</p>${rejected ? `<div class="curator-review-note"><strong>Pengajuan sebelumnya belum disetujui.</strong><p>${esc(application.adminNote || "Perbaiki informasi yang masih kurang, lalu kirim ulang.")}</p></div>` : ""}<form class="curator-form" data-curator-onboard-form>
      ...
      ${controlledTagPickerMarkup({ name:"profileTags", options:activeStyleOptions(), selected:application.profileTags || [], maximum:3, label:"Style utama", note:"Pilih hingga 3 style yang paling mewakili kurasimu." })}
      <div class="curator-field"><label for="curatorOnboardBio">Your style story</label><textarea id="curatorOnboardBio" name="bio" maxlength="500" placeholder="Style apa yang paling kamu suka? Ceritakan cara kamu memadukan outfit.">${esc(application.bio || "")}</textarea></div>
  ...
  function profileEditorMarkup(curator) {
    ...
    return `<section class="curator-studio-panel" data-curator-studio-panel="profile"><p class="eyebrow" style="color:var(--clay)">YOUR PUBLIC PROFILE</p><h3>Your profile.<br />Your point of view.</h3><p class="curator-studio-lede">Kenalkan style-mu lewat foto, bio, dan social links. Informasi ini akan tampil di profil publikmu.</p>
      ...
      ${controlledTagPickerMarkup({ name:"profileTags", options:activeStyleOptions(), selected:curator.jobTags, maximum:3, label:"Style utama", note:"Pilih hingga 3 style yang paling mewakili kurasimu." })}<div class="curator-field"><label for="curatorBio">Bio</label><textarea id="curatorBio" name="bio" maxlength="500" placeholder="Sudut pandang style kamu.">${esc(curator.bio)}</textarea></div>
  ...
  function productReferenceMarkup(item = {}, index = 0) {
    ...
    return `<div class="curator-product-reference" data-curator-reference-row>
      ...
      <div class="curator-field"><label>Harga saat kurasi (Rp)</label><input name="referencePrice" type="number" min="1" step="1" inputmode="numeric" value="${reference.price || ""}" placeholder="Contoh: 159000" required /></div>
      <div class="curator-field curator-field-full"><label>Link produk utama</label><input name="referenceUrl" type="url" value="${esc(reference.affiliateUrl)}" placeholder="https://shopee.co.id/... atau https://brand.com/product" required /><p class="curator-file-note">Shopee, TikTok Shop, dan website umum dikenali otomatis.</p></div>
  ...
  function lookEditorMarkup(editing = null) {
    ...
    return `<section class="curator-studio-panel" data-curator-studio-panel="editor"><p class="eyebrow" style="color:var(--clay)">${isExistingLook ? "EDIT LOOK" : "NEW CURATION"}</p><h3>${isExistingLook ? "Refine your\nlook." : "Let’s build\nyour look."}</h3><p class="curator-studio-lede">Upload foto, tambahkan 2–5 produk, lalu cek preview. Saat kamu pilih Terbitkan look, kurasimu langsung tampil di profil publik.</p>
      <form class="curator-form" novalidate data-curator-look-form data-curator-edit-id="${esc(editing?.id || "")}"><div class="curator-form-grid">
        <div class="curator-field curator-field-full"><label for="curatorLookTitle">Judul look</label><input id="curatorLookTitle" name="title" maxlength="160" value="${esc(editing?.title || "")}" placeholder="Contoh: Monday in Olive" required /></div>
        <div class="curator-field curator-field-full"><label for="curatorLookExcerpt">Deskripsi kurasi (opsional)</label><textarea id="curatorLookExcerpt" name="excerpt" maxlength="240" placeholder="Ceritakan vibe, occasion, atau tips mix & match look ini. Maksimal 240 karakter.">${esc(editing?.excerpt || "")}</textarea><p class="curator-file-note">Kosongkan jika judul dan visual sudah cukup menjelaskan look.</p></div>
  ...
  function setReferenceExpanded(row, expanded) {
    ...
    button.textContent = expanded ? "Tutup detail" : "Edit produk";
