...
check(read("assets/services/supabase.js").includes("storefront/${entry.cardKey}/") && read("assets/services/supabase.js").includes("2 * 1024 * 1024") && read("assets/components/image-optimizer.js").includes("cacheControl:'31536000'") && read("assets/services/supabase.js").includes("obsoletePaths") && read("assets/services/supabase.js").includes("COMOOTDImageOptimizer.upload"), "Upload, cache, batas ukuran, atau pembersihan desain storefront belum lengkap");
...
check(uiPolish.includes(":root:has(body.catalogue-route-open)") && uiPolish.includes(":root:has(body.curator-route-open)") && uiPolish.includes(":root:has(body.request-route-open)") && /body\.request-route-open\s*\{[^}]*position:\s*fixed;[^}]*overflow:\s*hidden;/s.test(uiPolish) && /overscroll-behavior-y:\s*contain/.test(uiPolish), "Route penuh masih dapat meneruskan scroll ke beranda di belakangnya");
